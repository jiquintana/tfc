...
groups = Table(
        ...
        Column('uid',Integer, ForeignKey('USER.uid', name='pertenece'), nullable=False, primary_key=True, index=True),
        Column('gid',Integer, ForeignKey('GROUP.gid', name='componentes'), nullable=False, primary_key=True, index=True),
...
class rules_uris(Base):
    ...
    #__table_args__ = (Index('idx_ruid' , 'ruid'), Index('idx_uri' , 'rule_uri'))
...
class rules_words(Base):
    ...
    #__table_args__ = (Index('idx_rwid' , 'rwid'),)
...
class _map_group2time(Base):
    ...
    #__table_args__ = (Index('idx_g2ti_rtid' , 'rtid'), Index('idx_g2ti_gid' , 'gid'))
...
class _map_group2uris(Base):
    __tablename__ = '_M_G2U'
    #__table_args__ = (Index('idx_g2ur_ruid' , 'ruid'), Index('idx_g2ur_gid' , 'gid'))
    ...
    rel_gid = relationship("GROUPS", backref="_map_group2uris",primaryjoin="_M_G2U.gid==GROUPS.gid")
    rel_rtid = relationship("RULES_URIS", backref="_map_group2uris", primaryjoin="_M_G2U.rtid==RULES_URIS.ruid")
    
    def __repr__(self):
        return "MAP G2U (%r:%r)" % (self.gid, self.ruid)

class _map_group2words(Base):
    __tablename__ = '_M_G2W'
    #__table_args__ = (Index('idx_g2wo_rwid' , 'rwid'), Index('idx_g2wo_gid' , 'gid'))
    ...
    rel_gid = relationship("GROUPS", backref="_map_group2uris",primaryjoin="_M_G2W.gid==GROUPS.gid")
    rel_rwid = relationship("RULES_WORDS", backref="_map_group2words", primaryjoin="_M_G2W.rwid==RULES_WORDS.rwid")
    
    def __repr__(self):
        return "MAP G2W (%r:%r)" % (self.gid, self.rwid)

class _map_user2group(Base):
    __tablename__ = '_M_U2G'
    #__table_args__ = (Index('idx_u2g_uid' , 'uid'), Index('idx_u2g_gid' , 'gid'))
